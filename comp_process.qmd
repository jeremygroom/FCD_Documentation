...
Download the program [Homebrew](https://brew.sh/). The webpage has directions.

Once installed, from the Terminal type: `brew install openssl`. R then needs to know where the SSL libraries are. In the Terminal, type: `brew list openssl@3`. This provides paths for different SSL objects In R, I enter `Sys.setenv(LDFLAGS = "-L/opt/homebrew/Cellar/openssl@3/lib")` . Your path (`/opt/homebrew/…/lib`) may differ.
...
Close and reopen RStudio and give R the command `renv::restore()` again. Hopefully everything loads. If not, an AI may be able to help out with a workaround for the issue.
